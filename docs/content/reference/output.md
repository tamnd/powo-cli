...
describes what users get. Pick a format with `-o`, or let powo choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
powo <command> -o table   # aligned columns for reading
powo <command> -o jsonl   # one JSON object per line, for piping
powo <command> -o json    # a single JSON array
powo <command> -o csv     # spreadsheet friendly
powo <command> -o tsv     # tab-separated
powo <command> -o url     # just the URL column
powo <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
powo <command> --fields id,title,url
```
...
```bash
powo <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
powo <command>            # a table, because this is a terminal
powo <command> | wc -l    # JSONL, because this is a pipe
```
